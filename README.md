# Blog · @v5ria WK

> 中文与 English 双语索引 / Bilingual index — 中文为原文，English is the translated version of the same piece.

---

## 🌐 Decentralized Society Trilogy / 去中心化社会三部曲

A 3-part essay series running from a quantum-physics metaphor (Wheeler's delayed-choice experiment) to a social architecture for the AI era.  
从量子物理的隐喻（惠勒延迟选择实验）出发，一路走到 AI 时代的社会架构，三篇为一个完整论证。

| # | Theme · 主题 | 中文 | English |
|---|---|---|---|
| 0 | Index · 索引 | [去中心化社会三部曲——索引](./去中心化社会三部曲——索引.md) | [Decentralized Society Trilogy — Index](./Decentralized-Society-Trilogy-Index.md) |
| 1 | Ontology / Why · 本体论 / 为什么 | [当下的观察决定过去——从惠勒延迟选择到去中心化社会](./当下的观察决定过去——从惠勒延迟选择到去中心化社会.md) | [Observation Now Decides the Past](./Observation-Now-Decides-the-Past.md) |
| 2 | Methodology / How it lands · 方法论 / 怎么落 | [分发观察权之后——去中心化在六个场景里的形状](./分发观察权之后——去中心化在六个场景里的形状.md) | [After Distributing Observation Rights](./Distributing-Observation-Rights.md) |
| 3 | Endgame / Why necessary in the AI era · 终局论 / 在 AI 时代为什么必须 | [AI 接管执行之后——一个新的基础社会架构](./AI接管执行之后——一个新的基础社会架构.md) | [After AI Takes Over Execution](./After-AI-Takes-Over-Execution.md) |

> **Reading order recommendation**: read in order 1 → 2 → 3 for the full argument; the index page (0) gives the through-line at a glance.  
> **建议阅读顺序**：1 → 2 → 3 顺序读为完整论证；索引页（0）一眼看清贯穿主线。

---

## 🤖 AI & Way of Working / AI 与工作方式

| 中文 | English | Notes · 备注 |
|---|---|---|
| [我是 AI 系统里最大的瓶颈](./我是AI系统里最大的瓶颈.md) | _(中文 only)_ | 当 Agent 越来越强，人在 AI 系统里慢慢成了瓶颈本身 |
| [局部自动化的错觉——从一杯奶茶到企业用 AI](./局部自动化的错觉——从一杯奶茶到企业用AI.md) | [The Illusion of Local Automation — From a Cup of Milk Tea to How Companies Use AI](./The-Illusion-of-Local-Automation.md) | 从一家奶茶店出发：每个环节都自动化，却仍靠人工串联，整体效率为何没提升 |
| [如何让 AI 真正自主运转](./如何让AI真正自主运转.md) | _(中文 only)_ | 给 AI 设计可自迭代任务的几条原则 |
| [别为人设计 AI 时代的工作流——七条新原则](./别为人设计AI时代的工作流——七条新原则.md) | [Stop Designing AI-Era Workflows for Humans — Seven New Principles](./Stop-Designing-AI-Era-Workflows-for-Humans.md) | 推翻人形流程、结果导向、Harness、24 小时、评估、并行 agent、FDE |
| [ML](./ML.md) | _(中文 only)_ | — |

---

## 💼 Career & Org / 职业与组织

| 中文 | English | Notes · 备注 |
|---|---|---|
| [AI 让一个人顶五个人之后——中层的三种动作](./AI让一个人顶五个人之后——中层的三种动作.md) | [After One Person Outproduces Five — Three Moves of the Middle](./After-One-Outproduces-Five-Three-Moves-of-the-Middle.md) | 借 Robert Dilts 的"中层三击"，复盘 AI 时代高产个体被中层定义的一次事件 |
| [前端工作质量](./前端工作质量.md) | _(中文 only)_ | — |
| [如何做好一次晋升准备](./如何做好一次晋升准备.md) | _(中文 only)_ | — |
| [微信团队分析](./微信团队分析.md) | _(中文 only)_ | — |
| [业务学习](./业务学习) | _(中文 only)_ | — |

---

## 🛠 Frontend / 前端

| 中文 | English | Notes · 备注 |
|---|---|---|
| [2015 前端趋势 - 浏览器篇](./2015前端趋势%20-%20浏览器篇.md) | _(中文 only)_ | — |
| [2015 趋势](./2015趋势.md) | _(中文 only)_ | — |
| _(English only)_ | [Manipulating the browser](./Manipulating-the-browser.md) | — |
| [ESLint 校验规则](./ESLint校验规则) | _(中文 only)_ | — |

---

## 🌍 Life / 生活

| 中文 | English | Notes · 备注 |
|---|---|---|
| [海外医疗](./海外医疗.md) | _(中文 only)_ | — |
| [健康长寿饮食范式——从餐次频率到细胞自噬与轻断食](./健康长寿饮食范式——从餐次频率到细胞自噬与轻断食.md) | _(中文 only)_ | 餐次频率、昼夜节律 TRE、大隅良典自噬理论与拟断食的系统整合 |
| [热海+伊豆高原+箱根——一次休息旅行的行程方案](./热海+伊豆高原+箱根——一次休息旅行的行程方案.md) | _(中文 only)_ | 从北京出发的热海+伊豆高原+箱根休息旅行完整方案，含航班、新干线班次、酒店、吃喝玩乐清单 |
| [热海+伊豆高原+箱根——出行打印清单](./热海+伊豆高原+箱根——出行打印清单.md) | _(中文 only)_ | 配套的精简随身版：交通预约指南、支付方式、打包清单 |

---

> Author · 作者: **@v5ria WK**  
> All Chinese pieces are the original; English versions, when present, are translations of the same essay with paragraph-level parity.  
> 所有中文均为原文；英文版（如果存在）与中文版段落级对应。
